#!/usr/bin/env python3
"""
Исправленная миграция данных с SQL Server в PostgreSQL
"""
...
class FixedDataMigration:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('fixed-migration.log'),
    ...
    def convert_data_type(self, mssql_type):
        ...
        type_mapping = {
            ...
            'uniqueidentifier': 'UUID',  # Добавлен UUID
            'uuid': 'UUID'
        }
    ...
    def migrate_table_data(self, table_name):
        ...
        try:
            ...
            # Пакетная вставка с обработкой UUID
            batch_size = 1000
            total_inserted = 0
    ...
    def create_group_report(self, group_name, tables, successful, failed):
        ...
        filename = f"fixed-migration-{group_name.lower().replace(' ', '-')}-report.json"
...
def main():
    """Основная функция"""
    print("🚀 Исправленная миграция данных с SQL Server в PostgreSQL")
    print("=" * 60)
    
    migrator = FixedDataMigration()
