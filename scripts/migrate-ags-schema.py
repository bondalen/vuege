#!/usr/bin/env python3
"""
Миграция всей схемы ags из SQL Server в PostgreSQL
"""
...
class AGSchemaMigration:
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('ags-schema-migration.log'),
    ...
    def get_ags_tables(self):
        ...
        cursor.close()
        logging.info(f"📋 Найдено {len(tables)} таблиц в схеме ags")
        return tables
    ...
    def migrate_ags_schema(self):
        """Миграция всей схемы ags"""
        logging.info("🎯 НАЧАЛО МИГРАЦИИ СХЕМЫ AGS")
        logging.info("=" * 60)
        ...
        try:
            ...
            for i, table_name in enumerate(tables, 1):
                logging.info(f"📋 [{i}/{len(tables)}] Обработка таблицы: {table_name}")
            ...
            logging.info(f"📈 Общий прогресс: {len(successful_tables)}/{len(tables)} ({len(successful_tables)/len(tables)*100:.1f}%)")
...
def main():
    """Главная функция"""
    print("🎯 МИГРАЦИЯ СХЕМЫ AGS ИЗ SQL SERVER В POSTGRESQL")
    print("=" * 60)
    
    migrator = AGSchemaMigration()
    successful, failed = migrator.migrate_ags_schema()
