...
# -*- coding: utf-8 -*-
"""
Скрипт для получения информации о колонках напрямую из таблицы mcl.mssql_columns
"""

import psycopg2

def get_columns_direct():
    """Получить информацию о колонках напрямую из таблицы"""
    ...
    try:
        ...
        print("=== ИНФОРМАЦИЯ О КОЛОНКАХ 10 ТАБЛИЦ (напрямую из таблицы) ===\n")
        
        for table in tables:
            print(f"📋 Таблица: {table}")
            print("Колонки:")
            
            # Прямой запрос к таблице без представлений
            query = f"""
            SELECT 
                c.column_name,
                mbt.base_type_name,
        ...
        conn.close()
        print("✅ Данные успешно получены напрямую из таблицы mcl.mssql_columns")
