#!/usr/bin/env python3
"""
Скрипт для поэтапной миграции данных из SQL Server в PostgreSQL
Мигрирует таблицы группами по 10-20 штук
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('staged-migration.log'),
...
class StagedMigration:
    def __init__(self):
        # Группы таблиц для поэтапной миграции
        self.table_groups = {
            'Группа 1 - Основные таблицы': [
                'accnt',      # Счета
                'cn',         # Контракты
                'cst',        # Клиенты
                'org',        # Организации
                'st',         # Статусы
                'yr',         # Годы
                'ra',         # Регистрационные данные
                'ipg',        # Инвестиционные группы
                'inv',        # Инвестиции
                'cnNum'       # Номера контрактов
            ],
            'Группа 2 - Контракты и документы': [
    ...
    def run_staged_migration(self):
        ...
        logging.info("🚀 Начало поэтапной миграции схемы ags")
