...
# -*- coding: utf-8 -*-
"""
Скрипт для получения информации о колонках 10 таблиц из PostgreSQL
для сверки с данными из MS SQL Server
"""

import psycopg2

def get_postgres_columns_info():
    """Получить информацию о колонках для 10 выбранных таблиц из PostgreSQL"""
    ...
    try:
        ...
        print("=== ИНФОРМАЦИЯ О КОЛОНКАХ 10 ТАБЛИЦ ИЗ POSTGRESQL ===\n")
        
        for table in tables:
            print(f"📋 Таблица: {table}")
            print("Колонки:")
            
            # Получение информации о колонках из mcl.mssql_columns
            query = f"""
            SELECT 
                c.column_name,
                mbt.base_type_name,
        ...
        conn.close()
        print("✅ Данные успешно получены из PostgreSQL")
        
    except Exception as e:
        print(f"❌ Ошибка при получении данных: {e}")

if __name__ == "__main__":
    get_postgres_columns_info()