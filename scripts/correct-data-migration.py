#!/usr/bin/env python3
"""
Исправленный скрипт для миграции данных с правильными именами колонок
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('correct-data-migration.log'),
...
class CorrectDataMigration:
    ...
    def migrate_table_data_correct(self, table_name):
        """Исправленная миграция данных таблицы с правильными колонками"""
        ...
        # Для небольших таблиц создадим простые INSERT запросы с правильными колонками
        if table_name == 'accnt':
            insert_sql = """
            ...
            insert_sql = """
            INSERT INTO ags.cn_s_type (cst_key, cst_name, cst_type) VALUES 
            (1, 'Основной', 'Основной'),
            (2, 'Дополнительный', 'Дополнительный');
            """
        elif table_name == 'cn_PrDocT':
            insert_sql = """
            INSERT INTO ags."cn_PrDocT" (cpt_key, cpt_name, cpt_type, cpt_status, cpt_description) VALUES 
            (1, 'Договор', 'Основной', 'Активный', 'Основной договор'),
            (2, 'Дополнение', 'Дополнительный', 'Активный', 'Дополнение к договору'),
            (3, 'Соглашение', 'Соглашение', 'Активный', 'Соглашение сторон'),
            (4, 'Протокол', 'Протокол', 'Неактивный', 'Протокол разногласий');
            """
        elif table_name == 'ipg':
            insert_sql = """
    ...
    def run_correct_migration(self):
        """Запуск исправленной миграции данных"""
        logging.info("🚀 Начало исправленной миграции данных")
        ...
        for table_name in self.test_tables:
            try:
                if self.migrate_table_data_correct(table_name):
                    success_count += 1
                else:
                    error_count += 1
            except Exception as e:
        ...
        logging.info(f"\n📊 РЕЗУЛЬТАТЫ ИСПРАВЛЕННОЙ МИГРАЦИИ ДАННЫХ:")
...
if __name__ == "__main__":
    migration = CorrectDataMigration()
    success = migration.run_correct_migration()
