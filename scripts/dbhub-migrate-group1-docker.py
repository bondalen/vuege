#!/usr/bin/env python3
"""
@file: dbhub-migrate-group1-docker.py
@description: Миграция данных группы 1 через Docker с использованием DBHub
@created: 2025-08-30
"""
...
class DBHubGroup1DockerMigration:
    def __init__(self):
        self.setup_logging()
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('dbhub-group1-docker-migration.log'),
    ...
    def insert_postgres_data(self, table_name, data_lines):
        ...
        try:
            ...
            for line in data_lines:
                if line.strip():
                    # Простая обработка строки (можно улучшить)
                    ...
                    if len(values) >= len(columns):
                        ...
                        # Вставка данных
                        pg_cursor.execute(insert_sql, values[:len(columns)])
            
            self.pg_conn.commit()
            logging.info(f"✅ Данные таблицы {table_name} мигрированы: {len(data_lines)} строк")
            return True
...
def main():
    """Основная функция"""
    print("🚀 Запуск миграции данных группы 1 через Docker с DBHub")
    
    migrator = DBHubGroup1DockerMigration()
