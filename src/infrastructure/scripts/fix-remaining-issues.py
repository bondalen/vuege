#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Исправление оставшихся проблемных таблиц схемы AGS
Автор: AI Assistant
Дата: 30 августа 2025
"""

import pymssql
import psycopg2
import psycopg2.extras
import logging
import sys
import os
from datetime import datetime

# Настройка логирования
def setup_logging():
    """Настройка логирования"""
    # Создаем папку logs если её нет
    logs_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'logs')
    os.makedirs(logs_dir, exist_ok=True)
    
    log_file = os.path.join(logs_dir, f'fix-remaining-{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler(log_file),
            logging.StreamHandler(sys.stdout)
        ]
    )
    return logging.getLogger(__name__)

# Оставшиеся проблемные таблицы
REMAINING_TABLES = [
    # Проблемы с длиной строк
    'ogAgFeeGr',
    
    # Проблемы с дублированием типов
    'cn_PrDocCs',
    'cn_s_orgExeBuirg',
    'cnInvAccntCs',
    'cstCcnGr',
    'dtqInvoice',
    'dym_pm_pm',
    'impPIR_21',
    
    # Проблемы с несуществующими объектами
    'dtqContract',
    'dtqCounterparty',
    'dtqInvoiceDbt',
    'RaRa_chSmLt_RltYy',
    
    # Проблемы с несуществующими колонками
    'cn_s_orgCustBuirgInv',
    'cn_s_orgExeBuirgInv',
    'TimeRsltCAP',
    'TimeRsltM',
    'TimeRsltMrl',
    'TimeRsltMrTtl',
    'yr_ctrl',
    'yr_ctrl_cm',
    'yr_ctrl_cm_21',
    'yr_ctrl_cmAcc21',
    'yr_ctrl_cmAcc21_06',
    'yr_ctrlAccnt',
    
    # Синтаксические ошибки
    'ra_period'
]

def connect_databases():
    """Подключение к базам данных"""
    try:
        # SQL Server
        mssql_conn = pymssql.connect(
            server='localhost',
            port=1433,
            user='sa',
            password='Vuege2024!',
            database='Fish_Eye'
        )
        
        # PostgreSQL
        pg_conn = psycopg2.connect(
            host='localhost',
            port=5432,
            database='vuege',
            user='postgres',
            password='postgres'
        )
        
        logging.info("✅ Подключения к базам данных установлены")
        return mssql_conn, pg_conn
        
    except Exception as e:
        logging.error(f"❌ Ошибка подключения к базам данных: {e}")
        raise

def check_table_exists(mssql_conn, table_name):
    """Проверка существования таблицы в SQL Server"""
    try:
        cursor = mssql_conn.cursor()
        query = f"""
        SELECT COUNT(*) 
        FROM INFORMATION_SCHEMA.TABLES 
        WHERE TABLE_NAME = '{table_name}' AND TABLE_SCHEMA = 'ags'
        """
        cursor.execute(query)
        result = cursor.fetchone()
        cursor.close()
        return result[0] > 0
    except Exception as e:
        logging.error(f"❌ Ошибка проверки существования таблицы {table_name}: {e}")
        return False

def get_table_schema_from_mssql(mssql_conn, table_name):
    """Получение схемы таблицы из SQL Server"""
    try:
        cursor = mssql_conn.cursor()
        query = f"""
        SELECT 
            COLUMN_NAME,
            DATA_TYPE,
            IS_NULLABLE,
            CHARACTER_MAXIMUM_LENGTH,
            NUMERIC_PRECISION,
            NUMERIC_SCALE
        FROM INFORMATION_SCHEMA.COLUMNS 
        WHERE TABLE_NAME = '{table_name}' AND TABLE_SCHEMA = 'ags'
        ORDER BY ORDINAL_POSITION
        """
        cursor.execute(query)
        columns = cursor.fetchall()
        cursor.close()
        
        schema = []
        for col in columns:
            column_info = {
                'name': col[0].lower(),
                'type': col[1],
                'nullable': col[2] == 'YES',
                'max_length': col[3],
                'precision': col[4],
                'scale': col[5]
            }
            schema.append(column_info)
        
        return schema
        
    except Exception as e:
        logging.error(f"❌ Ошибка получения схемы таблицы {table_name}: {e}")
        raise

def map_sql_to_pg_type(column_info):
    """Маппинг типов SQL Server в PostgreSQL"""
    sql_type = column_info['type'].lower()
    
    if sql_type in ['int', 'bigint', 'smallint', 'tinyint']:
        return 'integer'
    elif sql_type in ['decimal', 'numeric']:
        if column_info['precision'] and column_info['scale']:
            return f"numeric({column_info['precision']},{column_info['scale']})"
        return 'numeric'
    elif sql_type in ['varchar', 'nvarchar', 'char', 'nchar']:
        if column_info['max_length']:
            # Увеличиваем длину для проблемных таблиц
            if column_info['max_length'] < 1000:
                return f"varchar(1000)"
            else:
                return f"varchar({column_info['max_length']})"
        return 'varchar'
    elif sql_type in ['text', 'ntext']:
        return 'text'
    elif sql_type in ['datetime', 'datetime2', 'smalldatetime']:
        return 'timestamp'
    elif sql_type in ['date']:
        return 'date'
    elif sql_type in ['time']:
        return 'time'
    elif sql_type in ['bit']:
        return 'boolean'
    elif sql_type in ['uniqueidentifier']:
        return 'text'
    elif sql_type in ['money', 'smallmoney']:
        return 'numeric(19,4)'
    else:
        return 'text'

def create_table_in_pg_with_error_handling(pg_conn, table_name, schema):
    """Создание таблицы в PostgreSQL с обработкой ошибок"""
    try:
        cursor = pg_conn.cursor()
        
        # Генерация SQL для создания таблицы
        columns_sql = []
        for col in schema:
            pg_type = map_sql_to_pg_type(col)
            nullable = "" if col['nullable'] else " NOT NULL"
            columns_sql.append(f'"{col["name"]}" {pg_type}{nullable}')
        
        create_sql = f"""
        CREATE TABLE IF NOT EXISTS ags."{table_name.lower()}" (
            {', '.join(columns_sql)}
        )
        """
        
        cursor.execute(create_sql)
        pg_conn.commit()
        cursor.close()
        
        logging.info(f"✅ Таблица ags.{table_name.lower()} создана успешно")
        return True
        
    except Exception as e:
        error_msg = str(e)
        if "duplicate key value violates unique constraint" in error_msg and "pg_type_typname_nsp_index" in error_msg:
            logging.warning(f"⚠️ Тип уже существует для таблицы {table_name}, пропускаем создание типа")
            # Пытаемся создать таблицу без типа
            try:
                cursor = pg_conn.cursor()
                cursor.execute(f"DROP TABLE IF EXISTS ags.{table_name.lower()}")
                pg_conn.commit()
                cursor.close()
                return create_table_in_pg_with_error_handling(pg_conn, table_name, schema)
            except Exception as e2:
                logging.error(f"❌ Ошибка при повторной попытке создания таблицы {table_name}: {e2}")
                return False
        else:
            logging.error(f"❌ Ошибка создания таблицы {table_name}: {e}")
            pg_conn.rollback()
            return False

def migrate_table_data_with_error_handling(mssql_conn, pg_conn, table_name):
    """Миграция данных таблицы с обработкой ошибок"""
    try:
        # Получаем данные из SQL Server
        mssql_cursor = mssql_conn.cursor()
        query = f"SELECT * FROM ags.{table_name}"
        mssql_cursor.execute(query)
        rows = mssql_cursor.fetchall()
        mssql_cursor.close()
        
        if not rows:
            logging.info(f"📋 Таблица {table_name} пустая, пропускаем")
            return True
        
        # Получаем имена колонок
        mssql_cursor = mssql_conn.cursor()
        mssql_cursor.execute(f"SELECT * FROM ags.{table_name} WHERE 1=0")
        column_names = [desc[0].lower() for desc in mssql_cursor.description]
        mssql_cursor.close()
        
        # Вставляем данные в PostgreSQL
        pg_cursor = pg_conn.cursor()
        
        # Подготавливаем SQL для вставки
        placeholders = ', '.join(['%s'] * len(column_names))
        column_names_quoted = ', '.join([f'"{name}"' for name in column_names])
        
        insert_sql = f"""
        INSERT INTO ags."{table_name.lower()}" ({column_names_quoted})
        VALUES ({placeholders})
        """
        
        # Вставляем данные пакетами
        batch_size = 1000
        total_rows = len(rows)
        
        for i in range(0, total_rows, batch_size):
            batch = rows[i:i + batch_size]
            
            # Конвертируем данные
            converted_batch = []
            for row in batch:
                converted_row = []
                for value in row:
                    if isinstance(value, str):
                        converted_row.append(value)
                    elif value is None:
                        converted_row.append(None)
                    else:
                        converted_row.append(str(value))
                converted_batch.append(converted_row)
            
            pg_cursor.executemany(insert_sql, converted_batch)
            
            if i + batch_size < total_rows:
                logging.info(f"   📊 Прогресс: {i + batch_size}/{total_rows} записей")
        
        pg_conn.commit()
        pg_cursor.close()
        
        logging.info(f"✅ Данные таблицы {table_name} мигрированы успешно ({total_rows} записей)")
        return True
        
    except Exception as e:
        logging.error(f"❌ Ошибка миграции данных таблицы {table_name}: {e}")
        pg_conn.rollback()
        return False

def fix_remaining_table(mssql_conn, pg_conn, table_name):
    """Исправление оставшейся проблемной таблицы"""
    try:
        logging.info(f"🔧 Исправление таблицы: {table_name}")
        
        # Проверяем существование таблицы
        if not check_table_exists(mssql_conn, table_name):
            logging.warning(f"⚠️ Таблица {table_name} не существует в SQL Server, пропускаем")
            return True
        
        # Получаем схему из SQL Server
        schema = get_table_schema_from_mssql(mssql_conn, table_name)
        logging.info(f"   📋 Найдено {len(schema)} колонок")
        
        # Создаем таблицу в PostgreSQL
        if not create_table_in_pg_with_error_handling(pg_conn, table_name, schema):
            return False
        
        # Мигрируем данные
        if not migrate_table_data_with_error_handling(mssql_conn, pg_conn, table_name):
            return False
        
        logging.info(f"✅ Таблица {table_name} исправлена успешно")
        return True
        
    except Exception as e:
        logging.error(f"❌ Критическая ошибка при исправлении {table_name}: {e}")
        return False

def main():
    """Основная функция"""
    logger = setup_logging()
    
    logging.info("🔧 ИСПРАВЛЕНИЕ ОСТАВШИХСЯ ПРОБЛЕМНЫХ ТАБЛИЦ")
    logging.info("=" * 60)
    
    try:
        # Подключение к базам данных
        mssql_conn, pg_conn = connect_databases()
        
        # Обработка каждой оставшейся таблицы
        success_count = 0
        error_count = 0
        
        for i, table_name in enumerate(REMAINING_TABLES, 1):
            logging.info(f"📋 [{i}/{len(REMAINING_TABLES)}] Обработка таблицы: {table_name}")
            
            if fix_remaining_table(mssql_conn, pg_conn, table_name):
                success_count += 1
            else:
                error_count += 1
        
        # Закрытие соединений
        mssql_conn.close()
        pg_conn.close()
        
        # Итоговый отчет
        logging.info("=" * 60)
        logging.info("📊 ИТОГОВЫЙ ОТЧЕТ ИСПРАВЛЕНИЯ ОСТАВШИХСЯ ТАБЛИЦ")
        logging.info("=" * 60)
        logging.info(f"📋 Всего таблиц: {len(REMAINING_TABLES)}")
        logging.info(f"✅ Успешно исправлено: {success_count}")
        logging.info(f"❌ Ошибки исправления: {error_count}")
        logging.info(f"📈 Процент успеха: {(success_count/len(REMAINING_TABLES)*100):.1f}%")
        
        if error_count == 0:
            logging.info("🎉 ВСЕ ОСТАВШИЕСЯ ТАБЛИЦЫ ИСПРАВЛЕНЫ!")
            logging.info("🚀 ДОСТИГНУТО 100% МИГРАЦИИ СХЕМЫ AGS!")
        else:
            logging.warning(f"⚠️ Осталось {error_count} проблемных таблиц")
        
    except Exception as e:
        logging.error(f"❌ Критическая ошибка: {e}")
        sys.exit(1)

if __name__ == "__main__":
    main()