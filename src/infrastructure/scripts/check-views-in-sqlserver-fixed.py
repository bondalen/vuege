#!/usr/bin/env python3
"""
Скрипт для проверки объектов в SQL Server на предмет того, являются ли они представлениями
...
log_file = f"{log_dir}/check-views-in-sqlserver-fixed-{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# Параметры подключения к SQL Server (обновленные)
MSSQL_HOST = "localhost"
MSSQL_PORT = 1433
MSSQL_DB = "Fish_Eye"
MSSQL_USER = "sa"
MSSQL_PASSWORD = "Vuege2024!"  # Обновленный пароль
...
def analyze_object_types(sqlserver_objects):
    """Анализ типов объектов из SQL Server"""
    object_types = {}
    views = []
    tables = []
    other_objects = []
...
def check_migrated_views(sqlserver_objects, postgres_tables):
...
def main():
    """Основная функция"""
    logging.info("Начало проверки объектов в SQL Server")
    ...
    try:
        ...
        object_analysis = analyze_object_types(sqlserver_objects)
        
        # Проверка мигрированных представлений
        logging.info("Проверка мигрированных представлений...")
        migration_check = check_migrated_views(sqlserver_objects, postgres_tables)
        ...
        report_file = f"{report_dir}/check-views-in-sqlserver-fixed-report-{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        if migration_check['unknown_objects']:
            logging.info("Неизвестные объекты:")
            for obj in migration_check['unknown_objects']:
                logging.info(f"  - {obj['name']} (тип: {obj['type']})")
    ...
    logging.info("Проверка объектов в SQL Server завершена")
