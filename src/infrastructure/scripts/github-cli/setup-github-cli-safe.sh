...
# Безопасная настройка GitHub CLI с защитой от pager
# Использует Desktop Commander MCP подход

set -e

echo "🔧 Безопасная настройка GitHub CLI"
echo "=================================="

# Проверяем, установлен ли GitHub CLI
if ! command -v gh &> /dev/null; then
    echo "❌ GitHub CLI не установлен"
    echo "Сначала выполните: ./install-github-cli-safe.sh"
    exit 1
fi

echo "✅ GitHub CLI установлен"

# Проверяем статус аутентификации (с защитой от pager)
...
# Проверяем результат (с защитой от pager)
if gh auth status &> /dev/null; then
    echo ""
