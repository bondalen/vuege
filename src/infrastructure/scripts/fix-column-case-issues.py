...
# -*- coding: utf-8 -*-
"""
Исправление проблем с регистром имен колонок в схеме AGS
Автор: AI Assistant
...
def setup_logging():
    ...
    log_file = os.path.join(logs_dir, f'fix-column-case-{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
# Проблемные таблицы с именами колонок
PROBLEMATIC_TABLES = [
    'cn_PrDoc',
    'cn_PrDocT', 
    'cn_s_org_smplCs',
...
def get_table_schema_from_mssql(mssql_conn, table_name):
    ...
    try:
        ...
        for col in columns:
            column_info = {
                'name': col[0].lower(),  # Приводим к нижнему регистру
                'type': col[1],
                'nullable': col[2] == 'YES',
                'max_length': col[3],
                'precision': col[4],
...
def create_table_in_pg(pg_conn, table_name, schema):
    """Создание таблицы в PostgreSQL с исправленными именами колонок"""
    try:
        ...
        logging.info(f"✅ Таблица ags.{table_name.lower()} создана с исправленными именами колонок")
        return True
...
def migrate_table_data(mssql_conn, pg_conn, table_name):
    """Миграция данных таблицы с исправленными именами колонок"""
    try:
        ...
        column_names = [desc[0].lower() for desc in mssql_cursor.description]  # Приводим к нижнему регистру
        mssql_cursor.close()
            ...
            for row in batch:
                ...
                for value in row:
                    if isinstance(value, str):
                        ...
                        converted_row.append(str(value))  # Конвертируем в строку
...
def fix_table_column_case(mssql_conn, pg_conn, table_name):
    """Исправление проблем с регистром имен колонок для одной таблицы"""
    try:
        ...
        if not create_table_in_pg(pg_conn, table_name, schema):
            return False
        
        # Мигрируем данные
        if not migrate_table_data(mssql_conn, pg_conn, table_name):
            return False
...
def main():
    ...
    logging.info("🔧 ИСПРАВЛЕНИЕ ПРОБЛЕМ С РЕГИСТРОМ ИМЕН КОЛОНОК")
    logging.info("=" * 60)
    ...
    try:
        ...
        for i, table_name in enumerate(PROBLEMATIC_TABLES, 1):
            ...
            if fix_table_column_case(mssql_conn, pg_conn, table_name):
                success_count += 1
            else:
                error_count += 1
        ...
        # Итоговый отчет
        logging.info("=" * 60)
        logging.info("📊 ИТОГОВЫЙ ОТЧЕТ ИСПРАВЛЕНИЯ")
        logging.info("=" * 60)
        ...
        if error_count == 0:
            logging.info("🎉 ВСЕ ПРОБЛЕМЫ С РЕГИСТРОМ ИМЕН КОЛОНОК ИСПРАВЛЕНЫ!")
        else:
