...
# -*- coding: utf-8 -*-
"""
@file: migrate-fixable-tables.py
@description: Миграция таблиц, которые можно исправить и мигрировать
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'docs/infrastructure/logs/migrate-fixable-tables-{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def get_fixable_tables():
    """Получение списка таблиц, которые можно исправить и мигрировать"""
    return [
        # Таблицы с простыми проблемами, которые можно исправить
        'cn_PrDoc',
        'cn_PrDocP', 
        'cn_PrDocT',
        'invNum',
...
def migrate_table_safely(mssql_conn, pg_conn, table_name):
    """Безопасная миграция таблицы с обработкой ошибок"""
    try:
...
def main():
    ...
    logging.info("🔧 МИГРАЦИЯ ИСПРАВИМЫХ ТАБЛИЦ СХЕМЫ AGS")
    logging.info("=" * 80)
    ...
    try:
        fixable_tables = get_fixable_tables()
        success_count = 0
        error_count = 0
        
        for i, table_name in enumerate(fixable_tables, 1):
            logging.info(f"📋 [{i}/{len(fixable_tables)}] Миграция таблицы: {table_name}")
        ...
        # Итоговый отчет
        logging.info("=" * 80)
        logging.info("📊 ИТОГОВЫЙ ОТЧЕТ МИГРАЦИИ ИСПРАВИМЫХ ТАБЛИЦ")
        logging.info("=" * 80)
        logging.info(f"📋 Всего таблиц: {len(fixable_tables)}")
        logging.info(f"✅ Успешно мигрировано: {success_count}")
        logging.info(f"❌ Ошибки миграции: {error_count}")
        logging.info(f"📈 Процент успеха: {(success_count/len(fixable_tables)*100):.1f}%")
        
        if error_count == 0:
            logging.info("🎉 ВСЕ ИСПРАВИМЫЕ ТАБЛИЦЫ МИГРИРОВАНЫ УСПЕШНО!")
        else:
