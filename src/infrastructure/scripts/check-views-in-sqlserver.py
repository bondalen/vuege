...
log_file = f"{log_dir}/check-views-in-sqlserver-{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def main():
    ...
    try:
        ...
        report_file = f"{report_dir}/check-views-in-sqlserver-report-{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
