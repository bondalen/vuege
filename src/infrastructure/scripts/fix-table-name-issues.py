...
# -*- coding: utf-8 -*-
"""
Исправление проблем с именами таблиц (дефисы) в схеме AGS
Автор: AI Assistant
...
def setup_logging():
    ...
    log_file = os.path.join(logs_dir, f'fix-table-names-{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
...
def fix_table_name(table_name):
    """Исправление имени таблицы - замена дефисов на подчеркивания"""
...
def create_table_in_pg_with_fixed_name(pg_conn, original_name, schema):
    """Создание таблицы в PostgreSQL с исправленным именем"""
    try:
        ...
        create_sql = f"""
        CREATE TABLE IF NOT EXISTS ags."{fixed_name.lower()}" (
            {', '.join(columns_sql)}
        )
        """
        ...
        logging.info(f"✅ Таблица ags.{fixed_name.lower()} создана (исправлено: {original_name} -> {fixed_name})")
        return True
        
    except Exception as e:
        logging.error(f"❌ Ошибка создания таблицы {original_name}: {e}")
        pg_conn.rollback()
        return False

def migrate_table_data_with_fixed_name(mssql_conn, pg_conn, original_name):
    """Миграция данных таблицы с исправленным именем"""
    try:
        # Исправляем имя таблицы
        fixed_name = fix_table_name(original_name)
        ...
        query = f"SELECT * FROM ags.{original_name}"
        mssql_cursor.execute(query)
        rows = mssql_cursor.fetchall()
        mssql_cursor.close()
        
        if not rows:
            logging.info(f"📋 Таблица {original_name} пустая, пропускаем")
            return True
        
        # Получаем имена колонок
        mssql_cursor = mssql_conn.cursor()
        mssql_cursor.execute(f"SELECT * FROM ags.{original_name} WHERE 1=0")
        ...
        insert_sql = f"""
        INSERT INTO ags."{fixed_name.lower()}" ({column_names_quoted})
        VALUES ({placeholders})
        """
        ...
        logging.info(f"✅ Данные таблицы {original_name} мигрированы в {fixed_name} ({total_rows} записей)")
        return True
        
    except Exception as e:
        logging.error(f"❌ Ошибка миграции данных таблицы {original_name}: {e}")
        pg_conn.rollback()
        return False

def fix_table_name_issues(mssql_conn, pg_conn, table_name):
    """Исправление проблем с именами таблиц для одной таблицы"""
    try:
        logging.info(f"🔧 Исправление имени таблицы: {table_name}")
        ...
        # Создаем таблицу в PostgreSQL с исправленным именем
        if not create_table_in_pg_with_fixed_name(pg_conn, table_name, schema):
            return False
        
        # Мигрируем данные
        if not migrate_table_data_with_fixed_name(mssql_conn, pg_conn, table_name):
            return False
...
def main():
    ...
    logging.info("🔧 ИСПРАВЛЕНИЕ ПРОБЛЕМ С ИМЕНАМИ ТАБЛИЦ (ДЕФИСЫ)")
    logging.info("=" * 60)
    ...
    try:
        ...
        for i, table_name in enumerate(PROBLEMATIC_TABLES, 1):
            ...
            if fix_table_name_issues(mssql_conn, pg_conn, table_name):
                success_count += 1
            else:
                error_count += 1
        ...
        # Итоговый отчет
        logging.info("=" * 60)
        logging.info("📊 ИТОГОВЫЙ ОТЧЕТ ИСПРАВЛЕНИЯ ИМЕН ТАБЛИЦ")
        logging.info("=" * 60)
        ...
        if error_count == 0:
            logging.info("🎉 ВСЕ ПРОБЛЕМЫ С ИМЕНАМИ ТАБЛИЦ ИСПРАВЛЕНЫ!")
        else:
