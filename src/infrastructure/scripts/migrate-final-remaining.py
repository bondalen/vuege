...
# -*- coding: utf-8 -*-
"""
@file: migrate-final-remaining.py
@description: Финальная миграция оставшихся таблиц схемы AGS
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'docs/infrastructure/logs/migrate-final-remaining-{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def get_remaining_tables():
    """Получение списка оставшихся таблиц для миграции"""
    return [
        # Критические таблицы, которые нужно мигрировать
        'cn_inv_dbt_upl_g_p',
        'cn_inv_dbt_upl_pm_upl',
        'cn_inv_dbt_upl_pm_upl_g',
        'cn_inv_dbt_upl_pm_upl_g_p',
        'cn_inv_dbt_upl_pm_upl_g_p_r',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t_u',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t_u_v',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t_u_v_w',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t_u_v_w_x',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t_u_v_w_x_y',
        'cn_inv_dbt_upl_pm_upl_g_p_r_s_t_u_v_w_x_y_z'
    ]
...
def migrate_table_safely(mssql_conn, pg_conn, table_name):
    """Безопасная миграция таблицы"""
    try:
        ...
        # Проверяем существование таблицы в SQL Server
        ...
        if mssql_cursor.fetchone()[0] == 0:
            logging.warning(f"⚠️ Таблица {table_name} не найдена в SQL Server")
            return False
...
def main():
    ...
    logging.info("🔧 ФИНАЛЬНАЯ МИГРАЦИЯ ОСТАВШИХСЯ ТАБЛИЦ СХЕМЫ AGS")
    logging.info("=" * 80)
    ...
    try:
        ...
        for i, table_name in enumerate(remaining_tables, 1):
            logging.info(f"📋 [{i}/{len(remaining_tables)}] Миграция таблицы: {table_name}")
            
            success = migrate_table_safely(mssql_conn, pg_conn, table_name)
        ...
        # Итоговый отчет
        logging.info("=" * 80)
        logging.info("📊 ИТОГОВЫЙ ОТЧЕТ ФИНАЛЬНОЙ МИГРАЦИИ")
        logging.info("=" * 80)
        ...
        if error_count == 0:
            logging.info("🎉 ВСЕ ОСТАВШИЕСЯ ТАБЛИЦЫ МИГРИРОВАНЫ УСПЕШНО!")
