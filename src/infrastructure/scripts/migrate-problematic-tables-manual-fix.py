...
# -*- coding: utf-8 -*-
"""
@file: migrate-problematic-tables-manual-fix.py
@description: Миграция проблемных таблиц с ручным исправлением проблемных запросов
...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(f'docs/infrastructure/logs/migrate-problematic-tables-manual-fix-{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
def get_problematic_tables():
    """Получение списка проблемных таблиц для миграции"""
    return [
...
def migrate_table_with_existing_columns(mssql_conn, pg_conn, table_name):
    """Миграция таблицы только с существующими колонками"""
    try:
        mssql_cursor = mssql_conn.cursor()
        ...
        # Мигрируем данные только с существующими колонками
...
def main():
    ...
    logging.info("🔧 МИГРАЦИЯ ПРОБЛЕМНЫХ ТАБЛИЦ С РУЧНЫМ ИСПРАВЛЕНИЕМ")
    logging.info("=" * 80)
    ...
    try:
        ...
        for i, table_name in enumerate(problematic_tables, 1):
            ...
            success = migrate_table_with_existing_columns(mssql_conn, pg_conn, table_name)
