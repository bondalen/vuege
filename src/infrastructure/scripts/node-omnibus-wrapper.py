#!/usr/bin/env python3
"""
@file: node-omnibus-wrapper.py
@description: Python обертка для Node.js Omnibus MCP Server
...
def main():
    try:
        ...
        # Запускаем Node.js Omnibus MCP Server через npx
        cmd = ["npx", "node-omnibus"]
        subprocess.run(cmd, check=True)
        
    except subprocess.CalledProcessError as e:
        print(f"ERROR: Node.js Omnibus MCP Server failed with exit code {e.returncode}", file=sys.stderr)
